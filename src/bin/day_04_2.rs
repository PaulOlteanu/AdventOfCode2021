...
fn main() {
    ...
    let mut victor_number = 0u32;
    let mut victor = Board::new();
    while numbers.peek().is_some() {
        let number = numbers.next().unwrap();
        boards.iter_mut().for_each(|board| board.set_value(number));

        let victors: Vec<&Board> = boards.iter().filter(|&board| board.is_win()).collect();
        if !victors.is_empty() {
            victor = *victors[0];
            victor_number = number as u32;
        }
    ...
    println!("{:?}", victor);
    println!("Winning number: {:?}", victor_number);
    println!("Score: {:?}", victor_number as u32 * victor.sum_unfilled());
    println!("Finished");
}
