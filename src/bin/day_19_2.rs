...
fn main() {
    ...
    let mut largest = 0;
    for scanner1 in normalized_scanners.iter() {
        for scanner2 in normalized_scanners.iter() {
            if std::ptr::eq(scanner1, scanner2) {
                continue;
            }

            let off1 = scanner1.offset;
            let off2 = scanner2.offset;

            let dist = (off2.x - off1.x).abs() + (off2.y - off1.y).abs() + (off2.z - off1.z).abs();
            if dist > largest {
                largest = dist;
            }
        }
