...
fn reduce(list: &mut Vec<(u32, u32)>) {
    loop {
        ...
        split(list);
        println!("after split: {:?}", list);
    }
}

fn main() {
    let mut input: Vec<Vec<(u32, u32)>> = fs::read_to_string("inputs/day_18.txt")
        .unwrap()
        .trim()
        .lines()
        .map(|line| {
    ...
    let mut sum = input.remove(0);
    while !input.is_empty() {
        let pop = input.remove(0);
        let mut current = add(&sum, &pop);
        reduce(&mut current);
        sum = current;
    }
